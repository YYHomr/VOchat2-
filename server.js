...
// Check for AI tool call: VOAI file upload
const last = messages[messages.length - 1]?.content || "";

if (typeof last === "string" && last.includes("<voai-file")) {
  try {
    const match = last.match(/name="([^"]+)"/);
    const fileName = match ? match[1] : `file-${Date.now()}`;

    const base64 = last.replace(/^.*?>/, "").replace(/<\/voai-file>$/, "").trim();
    const buffer = Buffer.from(base64, "base64");
    ...
    const link = `${req.protocol}://${req.get("host")}/voai/download/${finalName}`;
    return res.json({
      ok: true,
      tool: "voai-file-upload",
      downloadURL: link
    });
  } catch (err) {
    return res.json({ error: "Invalid VOAI file block" });
  }
}
          ...
          try {
            ...
            if (args.type === 'pdf') {
              try {
                ...
                doc.fontSize(12).text(args.content);
                doc.end();
              }
...
app.post("/api/voai/upload-file", async (req, res) => {
  try {
    ...
    // Upload to your converter API
    const uploadResponse = await axios.post(
      "http://localhost:9999/upload",   // <-- change to your converter endpoint if different
      form,
